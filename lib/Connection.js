        ...
        $this.processBuffer(data, function(buffer) {
            ...
            if (packet.isFromServer()) {
                ...
                if (packet.isOk()) {
                    ...
                    $this.emit('command.error', $this.packets[seq].packet, packet);
                }
...
Connection.prototype.waitForHeader = function(data) {
    if (this.packetSize == 0) {
        var headerSize = Packet.HEADER_SIZE - this.packetOffset;
        if (headerSize > data.length) {
            data.copy(this.packetBuffer, this.packetOffset, 0);
            this.packetOffset += data.length;
        } else {
            data.copy(this.packetBuffer, this.packetOffset, 0, headerSize);
...
Connection.prototype.processBuffer = function(data, callback) {
   if ((data = this.waitForHeader(data)) && data.length) {
