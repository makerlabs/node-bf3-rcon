...
function Server(client, configuration) {
    ...
    this.on('player.onTeamChange', this._processTeamSquadChange);
    this.on('player.onSquadChange', this._processTeamSquadChange);
...
Server.prototype.connect = function(callback) {
    ...
    this.client.connect(function() {
        ...
        $this.client.login($this.configuration.password, function() {
            $this._update();
            $this._updateVars();    
            $this.updateTimer = setInterval(function() { $this._update() }, 10000);
...
Server.prototype.findPlayerByName = function(name, useEqual) {
    if (useEqual) {
        for (var i = 0; i < this.players.length; i++) {
...
Server.prototype._processEvents = function(event, packet) {   
    if (this._events[event]) {
        ...
        switch (event) {
            case 'player.onSpawn':
                var result = { name: words[0], teamId: words[1] };
            break;
            ...
            case 'player.onTeamChange':
                var result = { name: words[0], teamId: words[1], squadId: words[2] };
            break;
...
Server.prototype._processAdminCommands = function(message) {
    ...
    if (prefix == '/') {
        if (!this.isAdmin(message.player) && message.text[1] == '@') {
            ...
            switch (parameters[0]) {
                ...
                case '@swap':
                    if (parameters.length == 1) {
                        var player = this.findPlayerByName(message.player, true);
...
Server.prototype._processTeamSquadChange = function(player) {
    var current = this.findPlayerByName(player.name, true);

    if (current) {
        current.teamId = player.teamId;
        current.squadId = player.squadId;
    } 
};
